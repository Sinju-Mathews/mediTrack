...
const Navbar = () => {
  ...
  const handleMyPrescriptions = () => {
    navigate("/patient/myprescriptions"); // Navigate to "My Prescriptions"
  };

  const handleMedicalReports = () => {
    navigate("/patient/medicalreports"); // Navigate to "Medical Reports"
  };

  const handleProfile = () => {
    if (user) {
      navigate(`/patient/profile`); // Navigate to profile without ID in the URL
    }
  };
  ...
  return (
    <nav className="navbar">
      <div className="navbar-left">
        ...
        <button className="nav-button2 myprescriptions-button" onClick={handleMyPrescriptions}>
          My Prescriptions
        </button>
        <button className="nav-button2 medicalreports-button" onClick={handleMedicalReports}>
          Medical Reports
        </button>
        <button className="nav-button2 profile-button"  onClick={handleProfile}>
          My Profile
        </button>
      </div>
